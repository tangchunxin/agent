...
class PlayRechargeListFactory extends ListFactory
{
    ...
    public function __construct($dbobj, $aid = null, $id_multi_str='',$start_time = null, $end_time = null) 
    {
        ...
        elseif($aid == null && $id_multi_str == null && $start_time  && $end_time ) 
        {
            $this->key = $this->key.$aid;
            $this->sql = "select `play_rid` from `play_recharge` where `recharge_time` between ".$start_time." and " .$end_time ."";
            parent::__construct($dbobj, $this->key);
            return true;
        }
